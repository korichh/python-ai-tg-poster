...
- For each asset, include only these sections in this order:
  1. Context: 1–2 sentences summarizing the current market situation relevant to the coin.
...
- Write in clear, professional numbered list for each coin.
- Avoid filler, verbosity, or promotional language.
- Ensure all insights are precise, practical, and actionable.
- Total post length: 250–400 words (brief enough to read in ~1 minute).
- List coins one after another—avoid mixing data between coins."""
...
news_text_prompt = """Analyze the news provided on the context, pick top 3 news that affect the assets most, follow a clear structure to create concise, actionable insights.
...
- Total post length: 120–200 words (brief enough to read in ~1 minute).
...
BlackRock increased its stake in the BTC ETF by 7%. This strengthens the fundamental demand for Bitcoin."""
...
- **Length:** One or two sentences per joke.
